...
class RegisterController: UIViewController {
    ...
    func addSubControl() {
        ...
        //退出按钮
        dismissButton = UIButton(frame: CGRect(x: 20, y: 44, width: 21, height: 21))
        dismissButton.setImage(UIImage(named: "UMS_shake_close"), for: .normal)
        ...
        registerLabel = UILabel(frame: CGRect(x: label_x, y: 44, width: label_w, height: 21))
        ...
        logoImageView.image = UIImage(named: "logo-1_80x80_")
    ...
    func phoneTextField(phone_y: CGFloat) {
        phone = UITextField(frame: CGRect(x: 20, y: phone_y, width: screenSize.width - 20, height: 44))
        phone.placeholder = "手机号码（+86）"
        self.view.addSubview(phone)
    }
    
    func verificationCodeTextField(verificationCode_y: CGFloat) {
        verificationCode = UITextField(frame: CGRect(x: 20, y: verificationCode_y, width: screenSize.width - 20, height: 44))
        verificationCode.placeholder = "验证码"
        self.view.addSubview(verificationCode)
    }
    
    func theSeparator(theSeparator_y: CGFloat) -> UIView {
        let separator = UIView(frame: CGRect(x: 20, y: theSeparator_y, width: screenSize.width - 20, height: 0.5))
        separator.backgroundColor = UIColor(colorLiteralRed: 0.9, green: 0.9, blue: 0.9, alpha: 1)
        return separator
    }
    
    func nextStepButton(nextStep_y: CGFloat) {
        nextStep = UIButton()
        nextStep.frame.size = CGSize(width: 200, height: 50)
        nextStep.center = CGPoint(x: self.view.center.x, y: nextStep_y)
        nextStep.layer.cornerRadius = 25
        nextStep.layer.masksToBounds = true
        nextStep.setTitle("下一步", for: .normal)
        nextStep.titleLabel?.font = UIFont(name: "Helvetica-Bold", size: 17.0)
        nextStep.setTitleColor(UIColor.white, for: .normal)
        nextStep.backgroundColor = UIColor(red: 253/255.0, green: 190/255.0, blue: 27/255.0, alpha: 1)
    ...
    func toLogInButton(toLogIn_y: CGFloat) {
        ...
        toLogInButton.frame.size = CGSize(width: 200, height: 21)
        toLogInButton.center = CGPoint(x: self.view.center.x, y: toLogIn_y)
    ...
    func thirdPartyLoginTitle(title_y: CGFloat) {
        thirdPartyLoginLabel = UILabel()
        thirdPartyLoginLabel.frame.size = CGSize(width: 150, height: 21)
        thirdPartyLoginLabel.center = CGPoint(x: self.view.center.x, y: title_y)
        ...
        thirdPartyLoginLabel.textColor = UIColor(red: 210/255.0, green: 210/255.0, blue: 210/255.0, alpha: 1)
    ...
    func thirdPartyLoginLabel(x: CGFloat, y: CGFloat, title: String) -> UILabel {
        let label = UILabel(frame: CGRect(x: x, y: y, width: 50, height: 21))
