...
class LoginController: UIViewController {
    ...
    func addSubControl() {
        //退出按钮
        dismissButton = UIButton(frame: CGRect(x: 20, y: 44, width: 10, height: 18))
        dismissButton.setImage(UIImage(named: "back icon_10x18_"), for: .normal)
        ...
        loginLabel = UILabel(frame: CGRect(x: label_x, y: 44, width: label_w, height: 21))
    ...
    func passwordTextField(verificationCode_y: CGFloat) {
        password = UITextField(frame: CGRect(x: 20, y: verificationCode_y, width: screenSize.width - 20, height: 44))
    ...
    func loginButton(login_y: CGFloat) {
        login = UIButton()
        login.frame.size = CGSize(width: 200, height: 50)
        login.center = CGPoint(x: self.view.center.x, y: login_y)
        login.layer.cornerRadius = 25
        login.layer.masksToBounds = true
        login.setTitle("登录", for: .normal)
        login.titleLabel?.font = UIFont(name: "Helvetica-Bold", size: 17.0)
        login.setTitleColor(UIColor.white, for: .normal)
        login.backgroundColor = UIColor(red: 253/255.0, green: 190/255.0, blue: 27/255.0, alpha: 1)
        login.addTarget(self, action: #selector(loginEvent), for: .touchUpInside)
        self.view.addSubview(login)
    }
    
    func toRegisterButton(toLogIn_y: CGFloat) {
        toRegisterButton = UIButton()
        toRegisterButton.frame.size = CGSize(width: 200, height: 21)
        toRegisterButton.center = CGPoint(x: self.view.center.x, y: toLogIn_y)
